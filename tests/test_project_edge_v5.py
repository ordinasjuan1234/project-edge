...
def test_v5_uses_ema_regime_even_when_1h_structure_is_transition():
    decision = ProjectEdgeV5().decide_snapshot(
        snapshot("LONG", "A")
    )
...
def test_v5_blocks_strong_4h_opposition():
    ...
    assert (
        decision["checks"]["macro_4h_not_strongly_opposed"]
        is False
    )
...
def test_v5_blocks_extended_5m_trigger():
    ...
    assert decision["can_execute"] is False
    assert (
        decision["checks"]["trigger_not_extended"]
        is False
    )
...
def test_v5_fvg_changes_score_but_not_signal_requirement():
    ...
    plain = strategy.decide_snapshot(
        snapshot("LONG", "A")
    )
    ...
    enriched = strategy.decide_snapshot(
        enriched_value
    )

    assert (
        plain["can_execute"]
        is enriched["can_execute"]
        is True
    )

    assert enriched["quality_score"] == pytest.approx(
        plain["quality_score"] + 5.0
    )


def test_v5_cost_gate_rejects_trade_above_12_percent():
    strategy = ProjectEdgeV5()
    ...
    assert (
        plan["estimated_cost_risk_ratio"]
        > 0.12
    )

    assert (
        "Costo estimado excesivo"
        in plan["reason"]
    )


def test_v5_cost_gate_allows_trade_below_12_percent_and_keeps_x1():
    strategy = ProjectEdgeV5()

    # ATR 1.2 genera un stop suficientemente amplio
    # para que comision + slippage representen
    # menos del 12% del presupuesto de riesgo.
    decision = strategy.decide_snapshot(
        snapshot(
            "LONG",
            "A",
