...
def test_historical_config_accepts_v5_but_live_runner_remains_v3():
    ...
    assert isinstance(
        backtester.selected_strategy,
        ProjectEdgeV5,
    )

    assert isinstance(
        run_btc_paper.STRATEGY,
        ProjectEdgeV3,
    )
...
def test_portfolio_can_select_v5_without_changing_v4_default():
    ...
    v5 = PortfolioHistoricalBacktester(
        PortfolioHistoricalConfig(
            strategy=V5,
        )
    )
...
def ready_v5(
    ...
    return {
        ...
        # Con filtro costo/riesgo de 12%,
        # ATR 1.0 era insuficiente y el plan
        # quedaba correctamente rechazado.
        # Usamos 1.2 para probar una entrada
        # que sí cumple el nuevo límite.
        "atr_15m": 1.2,
...
def test_comparator_blocks_real_before_market_or_files(
    monkeypatch,
):
    monkeypatch.setenv(
        "PROJECT_EDGE_MODE",
...
def test_v5_outputs_include_setup_type(
    tmp_path,
):
    reports = {
        key: {
